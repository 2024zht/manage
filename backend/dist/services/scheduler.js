...
exports.startAttendanceScheduler = void 0;
...
async function createDailyTriggers() {
    try {
        ...
        const attendances = await new Promise((resolve, reject) => {
            db_1.db.all(`SELECT * FROM attendances 
         WHERE date(dateStart) <= date(?) 
         AND date(dateEnd) >= date(?)
         AND completed = 0`, [today, today], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
        ...
        for (const attendance of attendances) {
            try {
                ...
                const existingTrigger = await new Promise((resolve, reject) => {
                    db_1.db.get('SELECT id FROM daily_attendance_triggers WHERE attendanceId = ? AND triggerDate = ?', [attendance.id, today], (err, row) => {
                        if (err)
                            reject(err);
                        else
                            resolve(row);
                ...
                if (!existingTrigger) {
                    ...
                    await new Promise((resolve, reject) => {
                        db_1.db.run(`INSERT INTO daily_attendance_triggers (attendanceId, triggerDate, triggerTime, notificationSent, completed)
               VALUES (?, ?, ?, 0, 0)`, [attendance.id, today, triggerTime], (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
...
async function checkAndNotifyAttendances() {
    try {
        ...
        const triggers = await new Promise((resolve, reject) => {
         ...
         WHERE t.triggerDate = ?
         AND t.triggerTime <= ?
         AND t.notificationSent = 0`, [today, currentTime], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
        ...
        for (const trigger of triggers) {
            try {
                ...
                const attendance = await new Promise((resolve, reject) => {
                    db_1.db.get('SELECT targetGrades, targetUserIds FROM attendances WHERE id = ?', [trigger.attendanceId], (err, row) => {
                        if (err)
                            reject(err);
                        else
                            resolve(row);
                ...
                if (userEmails.length > 0) {
                    ...
                    // 发送邮件通知
                    await (0, email_1.sendAttendanceNotification)(userEmails, trigger.name, deadlineStr, trigger.locationName, trigger.latitude, trigger.longitude, trigger.radius);
                    // 标记为已发送通知
                    await new Promise((resolve, reject) => {
                        db_1.db.run('UPDATE daily_attendance_triggers SET notificationSent = 1 WHERE id = ?', [trigger.id], (err) => {
                            if (err)
                                reject(err);
                            else
                                resolve();
...
async function checkAndCompleteAttendances() {
    try {
        ...
        const triggers = await new Promise((resolve, reject) => {
            db_1.db.all(`SELECT t.*, a.name, a.penaltyPoints, a.createdBy
         FROM daily_attendance_triggers t
         JOIN attendances a ON t.attendanceId = a.id
         WHERE t.triggerDate = ?
         AND t.notificationSent = 1
         AND t.completed = 0`, [today], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
        ...
        for (const trigger of triggers) {
            try {
                ...
                // 获取已签到的用户
                const signedUsers = await new Promise((resolve, reject) => {
                    db_1.db.all('SELECT userId FROM attendance_records WHERE triggerId = ?', [trigger.id], (err, rows) => {
                        if (err)
                            reject(err);
                        else
                            resolve(rows || []);
                ...
                for (const user of unsignedUsers) {
                    try {
                        // 扣除积分
                        await new Promise((resolve, reject) => {
                            db_1.db.run('UPDATE users SET points = points - ? WHERE id = ?', [trigger.penaltyPoints, user.id], (err) => {
                                if (err)
                                    reject(err);
                                else
                                    resolve();
                        ...
                        // 记录积分日志
                        await new Promise((resolve, reject) => {
                            db_1.db.run(`INSERT INTO point_logs (userId, points, reason, createdBy) 
                 VALUES (?, ?, ?, ?)`, [
                                user.id,
                ...
                // 标记触发记录为已完成
                await new Promise((resolve, reject) => {
                    db_1.db.run('UPDATE daily_attendance_triggers SET completed = 1 WHERE id = ?', [trigger.id], (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
...
exports.default = {
    startAttendanceScheduler: exports.startAttendanceScheduler
};
