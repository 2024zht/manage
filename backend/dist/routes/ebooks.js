...
const fetchBooksFromWorker = async () => {
    ...
    try {
        const response = await axios_1.default.get(workerUrl + '/', {
            timeout: 5000, // 5秒超时
            headers: {
        ...
        const dates = Array.from(modifiedMatches).map((m) => m[1]);
...
const storage = multer_1.default.diskStorage({
    destination: function (req, file, cb) {
        const uploadDir = path_1.default.join(process.cwd(), 'uploads', 'ebooks');
...
const getLocalBooks = async () => {
    return new Promise((resolve, reject) => {
        db_1.db.all(`SELECT e.id, e.filename, e.originalName, e.fileSize, e.uploadedBy, 
              e.uploadedAt, e.b2Synced, e.b2Path, u.username as uploadedByUsername 
       FROM ebooks e 
       LEFT JOIN users u ON e.uploadedBy = u.id 
       ORDER BY e.uploadedAt DESC`, [], (err, rows) => {
            if (err)
                reject(err);
            else {
...
// 上传电子书（管理员）
router.post('/upload', auth_1.authenticateToken, auth_1.requireAdmin, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const insertId = await new Promise((resolve, reject) => {
            db_1.db.run('INSERT INTO ebooks (filename, originalName, fileSize, uploadedBy) VALUES (?, ?, ?, ?)', [file.filename, originalName, file.size, userId], function (err) {
                if (err)
                    reject(err);
                else
                    resolve(this.lastID);
        ...
        try {
            ...
            if (b2Bucket && b2Bucket !== 'robotlib' && b2Bucket.trim() !== '') {
                ...
                const { stdout, stderr } = await execAsync(`b2 upload-file "${b2Bucket}" "${localPath}" "${remoteFileName}"`, { timeout: 600000 } // 600秒超时（10分钟），支持大文件上传
                );
                console.log('B2 upload stdout:', stdout);
                if (stderr)
                    console.log('B2 upload stderr:', stderr);
                b2Synced = true;
                b2Path = `b2://${b2Bucket}/${remoteFileName}`;
                // 更新数据库
                await new Promise((resolve, reject) => {
                    db_1.db.run('UPDATE ebooks SET b2Synced = 1, b2Path = ? WHERE id = ?', [b2Path, insertId], (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
...
router.post('/cleanup-orphans', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const uploadsDir = path_1.default.join(process.cwd(), 'uploads', 'ebooks');
        // 确保目录存在
        ...
        const dbFiles = await new Promise((resolve, reject) => {
            db_1.db.all('SELECT filename FROM ebooks', [], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows);
        ...
        // 获取文件系统中的所有文件
        const fsFiles = fs_1.default.readdirSync(uploadsDir);
...
router.delete('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        // 获取电子书信息
        const ebook = await new Promise((resolve, reject) => {
            db_1.db.get('SELECT * FROM ebooks WHERE id = ?', [id], (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row);
        ...
        // 删除本地文件（如果存在）
        const localPath = path_1.default.join(process.cwd(), 'uploads', 'ebooks', ebook.filename);
        if (fs_1.default.existsSync(localPath)) {
            fs_1.default.unlinkSync(localPath);
        }
        // 从数据库删除
        await new Promise((resolve, reject) => {
            db_1.db.run('DELETE FROM ebooks WHERE id = ?', [id], (err) => {
                if (err)
                    reject(err);
                else
                    resolve();
...
router.get('/download/:filename', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (cfWorkerUrl && b2Bucket && b2Bucket !== 'robotlib' && b2Bucket.trim() !== '') {
            ...
            // 首先从数据库查找文件信息
            const ebook = await new Promise((resolve, reject) => {
                db_1.db.get('SELECT * FROM ebooks WHERE originalName = ? OR filename = ?', [decodeURIComponent(filename), decodeURIComponent(filename)], (err, row) => {
                    if (err)
                        reject(err);
                    else
                        resolve(row);
...
// 预览文件（在浏览器中打开，不下载）
router.get('/preview/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        // 从数据库获取文件信息
        const ebook = await new Promise((resolve, reject) => {
        ...
        // 构建文件路径
        const filePath = path_1.default.join(process.cwd(), 'uploads', 'ebooks', ebook.filename);
        // 检查文件是否存在
        if (!fs_1.default.existsSync(filePath)) {
            return res.status(404).json({ error: '文件不存在' });
        }
        // 根据文件扩展名设置 Content-Type
        const ext = path_1.default.extname(ebook.originalName).toLowerCase();
        const contentTypes = {
            '.pdf': 'application/pdf',
...
// 直接下载本地文件
router.get('/file/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        // 从数据库获取文件信息
        const ebook = await new Promise((resolve, reject) => {
