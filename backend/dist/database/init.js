    ...
    try {
      ...
      `, (err) => {
        ...
        const adminExists = await new Promise((resolve) => {
            db_1.db.get('SELECT id FROM users WHERE isAdmin = 1', (err, row) => {
                if (err || !row)
                    resolve(false);
                else
                    resolve(true);
        ...
        if (!adminExists) {
            ...
            await new Promise((resolve, reject) => {
                db_1.db.run('INSERT INTO users (username, name, studentId, className, email, password, isAdmin, points) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', ['admin', '系统管理员', 'ADMIN001', '管理员', 'admin@robotlab.com', hashedPassword, 1, 0], (err) => {
                    if (err)
                        reject(err);
                    else
                        resolve();
      ...
      `, (err) => {
        ...
        // 检查是否已有规则
        const rulesExist = await new Promise((resolve) => {
            db_1.db.get('SELECT id FROM rules LIMIT 1', (err, row) => {
                if (err || !row)
                    resolve(false);
                else
                    resolve(true);
        ...
        if (!rulesExist) {
            ...
            for (const rule of defaultRules) {
                await new Promise((resolve, reject) => {
                    db_1.db.run('INSERT INTO rules (name, points, description) VALUES (?, ?, ?)', [rule.name, rule.points, rule.description], (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
