...
const migrateAddGrade = async () => {
    try {
        ...
        await new Promise((resolve, reject) => {
            db_1.db.run('ALTER TABLE users ADD COLUMN grade TEXT', (err) => {
                if (err) {
                    // 如果字段已存在，忽略错误
        ...
        // 2. 将现有用户都设置为2023级
        await new Promise((resolve, reject) => {
            db_1.db.run('UPDATE users SET grade = ? WHERE grade IS NULL', ['2023'], (err) => {
                if (err)
                    reject(err);
                else {
        ...
        await new Promise((resolve, reject) => {
            db_1.db.run('ALTER TABLE attendances ADD COLUMN targetGrades TEXT', (err) => {
                if (err) {
                    if (err.message.includes('duplicate column name')) {
        ...
        await new Promise((resolve, reject) => {
            db_1.db.run('ALTER TABLE attendances ADD COLUMN targetUserIds TEXT', (err) => {
                if (err) {
                    if (err.message.includes('duplicate column name')) {
        ...
        await new Promise((resolve, reject) => {
            db_1.db.run('UPDATE attendances SET targetGrades = ? WHERE targetGrades IS NULL', [JSON.stringify(['2024', '2025'])], (err) => {
                if (err)
                    reject(err);
                else {
